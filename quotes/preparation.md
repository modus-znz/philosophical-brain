# Planning & Preparation

- **Core Principles**: [[preparation]] · [[prudence]] · [[first-principles]]
- **Session Moments**: planning/strategy · installing/gathering-tools
- **Tech Mappings**: [[api-contract-design]] · [[micro-saas-architecture]] · [[testing-strategy]]

> "Victorious warriors win first and then go to war." — [[Sun Tzu]] (*The Art of War*)

> "In all things, success depends on previous preparation." — [[Confucius]] (*Analects*)

> "A goal without a plan is just a wish." — attributed to [[Antoine de Saint-Exupéry]]

> "Measure twice, cut once." — carpenter's proverb

> "He who fails to plan, plans to fail." — proverb, after [[Benjamin Franklin]]

> "Strategy without tactics is the slowest route to victory; tactics without strategy is the noise before defeat." — attributed to [[Sun Tzu]]

> "Look before, or you'll find yourself behind." — [[Benjamin Franklin]] (*Poor Richard's Almanack*)

> "The wise man bridges the gap by laying out the path by means of which he can get from where he is to where he wants to go." — attributed to [[J.P. Morgan]]

> "Order and simplification are the first steps toward the mastery of a subject." — [[Thomas Mann]]

> "Kuuliza si ujinga." (Asking is not stupidity.) — Swahili proverb

> "Give me six hours to chop down a tree and I will spend the first four sharpening the axe." — attributed to [[Abraham Lincoln]]

> "Before enlightenment: chop wood, carry water. After enlightenment: chop wood, carry water." — Zen proverb

> "By failing to prepare, you are preparing to fail." — [[Benjamin Franklin]]

> "A craftsman who wishes to do his work well must first sharpen his tools." — [[Confucius]] (*Analects* 15.10)

> "Luck is what happens when preparation meets opportunity." — [[Seneca]]

> "Let us prepare our minds as if we'd come to the very end of life. Let us postpone nothing." — [[Seneca]] (*Letters* 101)

> "The time to repair the roof is when the sun is shining." — [[John F. Kennedy]]

> "Everything should be made as simple as possible, but not simpler." — attributed to [[Albert Einstein]]

> "Dig the well before you are thirsty." — Chinese proverb

## Code Directive

Plan and gather tooling before building. Reproduce the problem before fixing it,
define the contract before implementing, and set up the environment and smoke
test before release.
